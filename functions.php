...
function sweetmo_theme_setup() {
  ...
  add_action( 'wp_head', 'sweetmo_admin_ajax' );
...
function sweetmo_admin_ajax() {
  ...
  $out .= 'ajaxurl = ' . json_encode(admin_url( 'admin-ajax.php' )) . ';';
  $out .= '</script>';
...
/**
 * Setup registration magic
 */
require_once('include/registration.php');
SweetMoRegistration::instance()->add_ajax_callbacks();
add_action(
  "after_switch_theme",
  array(SweetMoRegistration::instance(), "configure"));
